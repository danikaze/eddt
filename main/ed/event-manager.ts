...
import { getJournalPath } from '@utils/get-journal';
import {
  EdNavRouteEvent,
  EdFSDJumpEvent,
  EdEvent,
...
import { ReadLineWatcher } from '@utils/read-line-watcher';
import { dataManager } from './data-manager';
import { msgLog, msgError } from '@main/utils/msgs';
...
class EventManager extends EventEmitter<EventType> {
  ...
  constructor(journalPath: string, options: Required<EventManagerOptions>) {
    ...
    if (this.options.verbose) {
      msgLog('Using journal file', basename(journalPath));
    }
  ...
  protected static parseEdEvent<E extends EventType>(
    ...
    try {
      ...
      msgError(`Error parsing event: ${json}`);
    }
  }
  ...
  protected static parseEdEventFile<E extends EventType>(
    ...
    try {
      ...
      msgError(`Error reading file: ${path}`);
      return;
    }
